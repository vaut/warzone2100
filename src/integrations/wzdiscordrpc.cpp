...
static void joinGameFromSecret_v1(const std::string joinSecretStr)
{
	...
	auto convertStringToPort = [](const std::string& portStr) -> unsigned int
	{
		unsigned int port = 0;
		try {
			unsigned long ulongValue = std::stoul(portStr);
			if (ulongValue > static_cast<unsigned long>(std::numeric_limits<unsigned int>::max()))
			{
				debug(LOG_WARNING, "Failed to convert string '%s' to unsigned int because of error: value is > std::numeric_limits<unsigned int>::max()", portStr.c_str());
			}
			port = static_cast<uint32_t>(ulongValue);
		}
		catch (const std::exception &e) {
			debug(LOG_WARNING, "Failed to convert string '%s' to unsigned int because of error: %s", portStr.c_str(), e.what());
		}
		return port;
	};
	...
	if (joinSecretComponents[1] == "l")
	{
		...
		if (joinSecretComponents.size() == 4)
		{
			...
			unsigned int lobbyPort = convertStringToPort(lobbyDetails[1].toUtf8());
			uint32_t lobbyGameId = static_cast<uint32_t>(std::stoul(joinSecretComponents[3].toUtf8()));
	...
	else if (joinSecretComponents[1] == "i")
	{
		...
		for (size_t i = 2; i < joinSecretComponents.size(); i++)
		{
			...
			uint32_t port = convertStringToPort(connectionDetails[1].toUtf8());
			if (port == 0)
			{
...
private:
	void setJoinInformation(const ActivitySink::MultiplayerGameInfo& info);
...
void DiscordRPCActivitySink::setJoinInformation(const ActivitySink::MultiplayerGameInfo& info)
{
	...
	else
	{
		...
		if (info.isHost)
		{
			...
			getPublicIPAddress([weakDiscordSink, listeningInterfaces](const std::string& ipv4Address, const std::string& ipv6Address, const std::vector<std::string>& errors) {
				...
				std::string joinSecretStr = std::string("v1/i/") + joinSecretDetails;

				// construct a unique party id from the ip addresses, hashed
				std::string rawPartyId = std::string("direct_connection:") + ipv4Address + ":" + std::to_string(listeningInterfaces.ipv4_port) + ":" + ipv6Address + ":" + std::to_string(listeningInterfaces.ipv6_port);
...
void discordRPCPerFrame()
{
	...
	if ((realTime - lastRPCUpdateTime) < (2 * GAME_TICKS_PER_SEC))
	{
		return;
	}
